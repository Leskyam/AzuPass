    ...
    protected System.Data.DataSet m_dsEntidad;
		...
		protected DataGen daCom()
		{
			if(null==this.m_daCom)
			{
				...
				string ServerTypeConfigured = Settings.dbServerType.ToUpper();

				if(ServerTypeConfigured == Settings.DataServerTypes.MSSql.ToString().ToUpper())
				{
					this.m_daCom = new DataMSSql();
				}
				else if(ServerTypeConfigured == Settings.DataServerTypes.MySQL.ToString().ToUpper())
				{
					this.m_daCom = new DataMySQL();
				}
				else if(ServerTypeConfigured == Settings.DataServerTypes.PostgreSQL.ToString().ToUpper())
				{
