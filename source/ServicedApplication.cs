	...
	this.ID = Id;
		...
		public bool isURLReachable()
		{
			try
			{
				...
				if(Settings.InternetAccess.proxyAddress!=null)
				{
					myProxy = new System.Net.WebProxy(Settings.InternetAccess.proxyAddress);
				...
				if(Settings.InternetAccess.proxyUser.Length!=0 & Settings.InternetAccess.proxyPasswd.Length!=0)
				{
					proxyCredentials.UserName = Settings.InternetAccess.proxyUser;
					proxyCredentials.Password = Settings.InternetAccess.proxyPasswd;
					if(Settings.InternetAccess.NetBIOSDomain.Length!=0)
					{
						proxyCredentials.Domain = Settings.InternetAccess.NetBIOSDomain;
					}
				}
				else
				{
